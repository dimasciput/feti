...
# base classes for GeoDjango admin classes
class GeoStackedInline(admin.StackedInline):
    """
    The administration options class for Geographic models. Map settings
    may be overloaded from their defaults to create custom maps.
    """
    # properties from grappeli's StackedInline:
    ...
    def _media(self):
        ...
        media = super(GeoStackedInline, self)._media()
    ...
    def get_map_widget(self, db_field):
        ...
        class OLMap(self.widget):
            ...
            params = {'default_lon': self.default_lon,
                      ...
                      'is_linestring': db_field.geom_type in (
                          'LINESTRING',
                          'MULTILINESTRING'),
                      'is_polygon': db_field.geom_type in (
                          'POLYGON',
                          'MULTIPOLYGON'),
                      'is_point': db_field.geom_type in (
                          'POINT',
                          'MULTIPOINT'),
...
if gdal.HAS_GDAL:
    class OSMGeoStackedInline(GeoStackedInline):
...
class NestedGeoModelAdmin(admin.ModelAdmin):
    """
    The administration options class for Geographic models. Map settings
    may be overloaded from their defaults to create custom maps.
    """
    ...
    def media(self):
        "Injects OpenLayers JavaScript into the admin."
        media = super(NestedGeoModelAdmin, self).media
        media.add_js([self.openlayers_url])
        media.add_js(self.extra_js)
        return media
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        ...
        if isinstance(db_field, models.GeometryField) and db_field.dim < 3:
            ...
            return super(NestedGeoModelAdmin, self).formfield_for_dbfield(
                db_field, **kwargs)
...
class NestedOSMGeoAdmin(NestedGeoModelAdmin):
    map_template = 'gis/admin/osm.html'
    num_zoom = 20
    map_srid = spherical_mercator_srid
    max_extent = '-20037508,-20037508,20037508,20037508'
